...
def flat_adm_quantities():
    ...
    # for the lapse and the shift and derivatives
    global trK, alpha, betaU, alpha_dD
    ...
    # Define alpha_dD and compute it symbolically
    alpha_dD = ixp.zerorank1()
    for i in range(DIM):
        alpha_dD[i] = sp.diff(alpha, coords[i])
    ...
    for i in range(DIM):
        alpha_dD[i] = sympify_integers__replace_rthph_or_Cartxyz(alpha_dD[i], coords, rfm.xxSph)
...
def kerr_adm_quantities():
    ...
    CoordSystem = par.parval_from_str("reference_metric::CoordSystem")
    if not "Spherical" in CoordSystem:
        print(f"Error: CoordSystem = {CoordSystem} unsupported!")
        sys.exit(1)
    ...
    # Compute the determinant of gamma
    detgamma = sp.sympify(1)
    for i in range(DIM):
        detgamma *= gammaDD[i][i]

    # Compute \hat{gamma} / \gamma
    detgamma_ratio = rfm.detgammahat / detgamma

    # Convert gammaDD to gammabarDD
    gammabarDD = ixp.zerorank2()
    for i in range(DIM):
        for j in range(DIM):
            gammabarDD[i][j] = detgamma_ratio**sp.Rational(1,3) * gammaDD[i][j]
    ...
    # Define the derivatives of gammabarDD
    # needed to compute the Christoffel symbols of the conformal 3-metric
    gammabarDD_dD = ixp.zerorank3()
    for i in range(DIM):
        for j in range(DIM):
            for k in range(DIM):
    ...
    # Compute the Christoffel symbols of the conformal 3-metric
    GammabarUDD = ixp.zerorank3()
    for i in range(DIM):
        for j in range(DIM):
            for k in range(DIM):
    ...
    # Define global variables for the trace of the extrinsic curvature,
    # the lapse and shift, the conformal factor, and derivatives
    global trK, phi, exp_m4phi, phi_dD, alpha, alpha_dD, betaU
    ...
    # Compute the conformal factor phi as 1/12 * \log(\gamma / \bar{\gamma})
    # We use \bar{\gamma} = \hat{\gamma} so we don't have to compute another determinant
    phi = sp.Rational(1,12) * sp.log(1/detgamma_ratio)

    # Define and compute e^{-4 \phi}
    exp_m4phi = sp.exp(-4 * phi)

    # Define and compute the derivatives of phi
    phi_dD = ixp.zerorank1()
    for i in range(DIM):
        phi_dD[i] = sp.diff(phi, coords[i])
    ...
    # Define betaU and set it's only non-zero component
    betaU = ixp.zerorank1()
    betaU[2] = - 2 * M * a * rBL / AA
    ...
    for i in range(DIM):
        ...
        betaU[i] = sympify_integers__replace_rthph_or_Cartxyz(
            betaU[i], coords, rfm.xxSph)
        phi_dD[i] = sympify_integers__replace_rthph_or_Cartxyz(phi_dD[i], coords, rfm.xxSph)
        for j in range(DIM):
            gammabarDD[i][j] = sympify_integers__replace_rthph_or_Cartxyz(
                gammabarDD[i][j], coords, rfm.xxSph)
            gammabarUU[i][j] = sympify_integers__replace_rthph_or_Cartxyz(
...
def print_gmq_quantities():
    ...
    # print('Printing gammaDD...')
    # for i in range(DIM):
    #     for j in range(DIM):
    #         print(f'gammaDD[{i}][{j}] = {gammaDD[i][j]}')
    # print('\n')

    # print('Printing gammaUU...')
    # for i in range(DIM):
    #     for j in range(DIM):
    #         print(f'gammaUU[{i}][{j}] = {gammaUU[i][j]}')
    # print('\n')

    # print('Printing GammaUDD...')
    # for i in range(DIM):
    #     for j in range(DIM):
    #         for k in range(DIM):
    #             print(f'GammaUDD[{i}][{j}][{k}] = {GammaUDD[i][j][k]}')
    # print('\n')
